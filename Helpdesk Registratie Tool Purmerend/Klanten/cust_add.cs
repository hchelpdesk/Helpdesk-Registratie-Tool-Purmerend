...
namespace Helpdesk_Registratie_Tool_Purmerend.Klanten
{
    public partial class cust_add : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(cust_add_txtbx_voornaam.Text))
                {
                    MessageBox.Show("Voornaam is verplicht!");
                }
                if (string.IsNullOrWhiteSpace(cust_add_txtbx_achternaam.Text))
                {
                    MessageBox.Show("Achternaam is verplicht!");
                }
                if (string.IsNullOrWhiteSpace(cust_add_txtbx_adres.Text))
                {
                    MessageBox.Show("Adres is verplicht!");
                }
                if (string.IsNullOrWhiteSpace(cust_add_txtbx_email.Text))
                {
                    MessageBox.Show("Emailadres is verplicht!");
                }
                if (string.IsNullOrWhiteSpace(cust_add_txtbx_telefoon.Text))
                {
                    MessageBox.Show("(Interne) Telefoon nummer is verplicht!");
                }
                if (string.IsNullOrWhiteSpace(cust_add_txtbx_afdeling.Text))
                {
                    MessageBox.Show("Afdeling / Ruimte nummer is verplicht!");
                }
                else
                {
